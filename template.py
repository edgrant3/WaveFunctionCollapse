...
class Template():
    def __init__(self, tileset_name, height, width) -> None:
    ...
    def load(self, file):
        loaded_template   = json.load(file)
        self.tileset.name = loaded_template["tileset_name"]
        self.h            = loaded_template["height"]
        self.w            = loaded_template["width"]
        self.data_raw     = np.array(loaded_template["data"])
        # self.data_encoded = loaded_template["analyzed_template"]["data"]
        self.data_encoded = {key: np.array(val) for key, val in loaded_template["analyzed_template"]["data"].items()}
